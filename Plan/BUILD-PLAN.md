...
| **G1 checkpoint** | **Fri 15:30 (mandatory — moved from 13:00 per request)** | Binding cut-fire, raised bar for the later time: is the merged foundation integrated **and the core loop demonstrably on track**? **NO → fire cut-lines 1–2 (C-flow demo, temporal extras) and collapse the third Fetch agent in-process** (E1 two-agent answer + the 0.5 h "three agents" doc sweep). The 12:30 merge (M-1) is now the *early-warning read*; the binding cut moves here. ⚠️ **Trade-off (flagged, not hidden):** this shrinks the recovery window before the immovable 18:00 gate from ~5 h to **2.5 h**, and puts G1 only 1.5 h ahead of G2 — so a cut fired at 15:30 must be surgical, and the team should treat the 12:30 merge read as the real steering signal rather than waiting for 15:30 to notice trouble |
...
| M-1 | 12:30–13:00 | Merge #1 with T2; **freeze the contracts boundary** (contracts.py, memory public API, SSE event schema); **G1 early-warning read** (binding cut-fire now 15:30) | — |
| G1 | 15:30 (brief pause during T1-4/T2-2) | Binding G1 cut decision — see §1 | — |
...
| N1-3 | 13:00–14:00 | **Manual-loop time-lapse**: perform the manual fix against the sim once (surfaces exist post-merge, ~13:00), screen-record, 8× in QuickTime, burn "8 hrs 51 min" → `00-raw/` |
...
- **The 18:00 gate has no buffer** — I removed the "CONFIRMED ACHIEVABLE" claim the first draft made; the honest position is that 18:00 is tight and the 15:30/17:00 checkpoints are the mechanical protection. If the morning slips, the binding cut-lines fire at 15:30 (with the 12:30 merge as the early-warning read), not silently at 18:00. ⚠️ **Moving G1 from 13:00 to 15:30 (per request) trades safety margin for a more-informed cut:** the recovery window before the immovable 18:00 gate is now 2.5 h, not 5 h. The mitigation is to steer off the 12:30 merge read — if integration looks shaky at the merge, pre-fire the cheapest cuts then rather than waiting for the 15:30 deadline.
...
**Post-plan edit (per request):** G1 mandatory cut-fire moved **13:00 → 15:30**. The 12:30 merge becomes an early-warning read; no task durations or per-person hours change (G1 is a decision point, not a work block), but the recovery buffer before the 18:00 gate drops from ~5 h to 2.5 h and G1 now sits 1.5 h before G2 — flagged in the G1 gate row and §8. The separate 13:00/19:00 Claude usage-check ritual is unchanged (it tracks the 5-hour rolling windows, not the gate).
