...
entry:
  ...
  generator: <T1|T2|T3|N1|N2>               # who runs it in their own AI tool and commits it
  when: <gate/phase anchor + hard deadline>  # phase-based, not a clock
  inputs: <named repo files to point the tool at>
  prompt: <a paste-ready starting prompt — adapt as needed; you have a capable model>
  output: <repo path(s)>
  consumers: <who uses it, at which gate>
  fallback: <what happens if the slot slips>
```
...
2. **Everyone commits their own output.** All five have repo access and a capable AI tool, so each generator runs their prompt directly in their AI tool (against the named repo files) and commits the result to a branch. No relay, no bundling, no attachment limits — point the tool at the files in-repo. The prompts below are paste-ready *starting points*; adapt them, you have a good model.
3. **No seat rationing.** Any generator can run any of these — the prompts are self-contained. If one model rate-limits, switch to your alternate (Codex/another provider). Every entry still has a slot-slips fallback so nothing is single-threaded on one person.
...
6. **Output locations:** everything lands under `Prep/`. Each generator commits their own output.
...
### 0.3 Dependency order (phase-anchored, not a clock)

```
Before the freeze     N2 generates PREP-5 (table-read agenda) ──► commit
Build stable, Fri eve TABLE READ runs from Prep/table-read-agenda.md (all five)
G2 CODE FREEZE
Just after freeze     T1 generates PREP-2A (both run-of-show variants) ──► commit
After the capture     T2 generates PREP-1 (systems walkthrough) ──► commit, incl. Prep/final-numbers.md
Sat, pre-Demo-Day     N1 generates PREP-3 (cribs ×5) using final-numbers.md ──► commit + print
Sat, pre-Demo-Day     T3 runs PREP-4 update passes (qa-bank, primer, explainer) ──► commit; patch cribs from the diff
At G5 decision        T1 executes PREP-2B (mechanical selection of the run-of-show variant, no model call)
```

The only hard ordering: PREP-1's `final-numbers.md` must exist before PREP-3 (cribs quote the numbers) and PREP-4 (the update pass reconciles them). Everything else is "whoever's free runs it in their AI tool and commits." If Saturday runs tight, the crib patch is the drop (cribs are already correct on the frozen numbers; the patch is belt-and-braces).

These are real work, not improvisation — generating them from the specs beats five people writing the same documents from scratch at the worst possible hours.
...
generator: T2 (directly), one AI-tool session over the frozen repo; commits it, and flags in the team
           channel that final-numbers.md is ready for the Saturday prep docs
when: right after the G2 capture session, Friday evening — this is the one sanctioned late task, because
      the numbers can't be final until the build is frozen and the bench has run
...
consumers: T2 (slide-8 build + tech Q&A owner, Sat); deck builder (slide 8 + PDF export);
           PREP-3 and PREP-4 (final-numbers.md); BasedAI PR README cross-check (G4)
fallback: if the model rate-limits mid-run, T2 finishes on their alternate (Codex) for the mechanical
...
>
> Also write `Prep/final-numbers.md` — a ONE-PAGE reality card the Saturday crib and update passes read from (keep it under ~150 lines):
...
generator: Stage A: T1 (directly), one AI-tool session.
           Stage B: T1, by hand — NO Claude call.
committer: T1 (directly); Stage B final copy goes to both presenters' phones + one print
when: Stage A: Fri 21:15–22:00, pinned to G4 (freeze at 21:00 means clicks/UI/hotkeys are final;
      ...
      Stage B: Sat 09:30–09:40, immediately after the G5 rehearsal-gate decision, before travel.
...
consumers: both presenters (P1 = T1 voice, P2 = T2 hands) at G5 rehearsal and on stage;
...
generator: N1 (in their own AI tool, directly), commits Prep/crib-sheets/, prints 2 copies of each
...
when: Sat 07:00–07:50 (N1's 06:30–09:30 window), pinned between G4 outputs (final-numbers.md
      exists from PREP-1) and G5 (cribs must be in hands before the rehearsal gates).
      Consistency patch: 08:15, T3 pen-patches printed cribs from PREP-4's qa-bank diff.
inputs (point your AI tool at these repo files): Prep/qa-bank.md, Prep/final-numbers.md (from PREP-1),
        Prep/tech-explainer.md. The Q&A role assignments from 03 and the G0 named-person grid go inline
        in the prompt.
output: Prep/crib-sheets/crib-T1.md, crib-T2.md, crib-T3.md, crib-N1.md, crib-N2.md
consumers: all five, on stage Sat + at the G5 gate session; N2's crib is also the cross-assignment
           card for Q&A
fallback: if N1's session slips, anyone free finishes the remaining cribs with the same prompt; if the
          whole thing fails, the table-read notes (PREP-5 output) + printed A1 serve as degraded cribs
          — flag it and don't improvise.
```
...
**Starting prompt (adapt as needed; you have the repo files and a capable model):**
...
generator: T3 (in their own AI tool, directly) — one short pass per doc, order fixed:
           qa-bank FIRST, then tech-explainer, then industry-primer; commits all three + the diff
when: Sat, pinned after PREP-1 (needs Prep/final-numbers.md) and before G4 (BasedAI PR
      final-ready — the refreshed qa-bank informs the PR's Q&A-facing README lines)
      and G5 (the rehearsal gates).
inputs: per pass: the one doc being refreshed + Prep/final-numbers.md
output: the three refreshed docs in place + Prep/update-pass-diff.md
consumers: crib patch 08:15; anyone answering Q&A Sat; G6 DoraHacks final-submit text
...
fallback: the passes are independent — if T3 is interrupted after the qa-bank pass, the other two docs
          can go untouched (they are less volatile) and only update-pass-diff.md (qa-bank rows) is
          required downstream. If even the qa-bank pass slips, anyone free runs this prompt.
```
...
```yaml
id: PREP-5
deliverable: A tight 25-minute table-read agenda incl. the 15 preselected questions,
             the deliberate cross-assignment beat, a scoring rubric, and N2's provisional bridge card
generator: N2 (in their own AI tool, directly) — N2 also facilitates the read itself
when: generated before the freeze, once the build is stable enough for the second run-through, so the
      question emphasis reflects any fired cut-lines; used at the table read (Fri evening), between the
      run-through and the freeze — the builders' hands are off the keyboard while the last push settles,
      and the read needs the run-through fresh in memory
inputs (point your AI tool at these repo files): Prep/qa-bank.md, Prep/tech-explainer.md, and the Q&A
        assignments + banned/required words + the two must-remember lines from 03-pitch-deck.md.
        The stage-role mapping (same as PREP-3) goes inline in the prompt
output: Prep/table-read-agenda.md ; Prep/table-read-notes.md (scribe template, part of the output)
consumers: all five at the table read; the notes feed PREP-3 (which questions wobbled → crib emphasis)
           and PREP-4 (answers that need sharper wording in qa-bank)
fallback: if the agenda isn't ready, hand-write a 10-line version: 3 questions per person from the
          bank's headings + the one cross-assignment below — the read happens regardless; the agenda
          is an efficiency aid, not a gate.
```

**Starting prompt (adapt as needed; point your AI tool at the repo files above):**
...
1. **Named-person grid → PREP-3/PREP-5 mapping.** The default stage-role table in PREP-3 is confirmed or overridden at G0; whatever the grid says is what goes into both prompts. Profiles only, on paper and in prompts.
2. **N1's 20:15 video call** is agreed at G0 (25 min outside N1's Friday shift — it is the only out-of-shift ask in this spec).
3. **PREP-1 feeds everything Saturday.** If the post-capture session produces nothing else, it must produce `Prep/final-numbers.md` — without it, PREP-3 and PREP-4 can't run and the cribs ship on Friday-afternoon numbers (legal but weaker).
