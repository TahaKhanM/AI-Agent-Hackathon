...
2. **Measured baseline from the real corpus (computed)**: from the UCI log, compute median `opened_at → resolved_at` for incidents in repeat categories (fingerprint seen ≥2 times) and — the killer stat — the median for incidents where `knowledge=true` (a documented fix existed) vs not. Put the actual computed number on the before/after bar: *"in 24,918 real incidents, repeat-class incidents with a known fix still took a median of X hours"*. This is OUR number from real data, unattackable in Q&A. (~30 min of pandas once ingest lands.)
