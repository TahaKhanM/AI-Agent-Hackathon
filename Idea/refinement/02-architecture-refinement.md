...
**Poll, don't webhook** (technical judge): webhooks need a public tunnel to a laptop on venue Wi-Fi. A 2–3 s poll of (a) JSM project role/permission-scheme membership, (b) permission-scheme grants on the live scheme (id 10000), and (c) **issue-security levels on runbook issues** (Standard-verified 3 Jul: scheme "Precedent Restricted Runbooks"=10000, levels Rights-Ops-Only=10000 / Scheduling-Ops-Only=10001, associated with MEDIA — restricted KB articles are mirrored as MEDIA "runbook" issues whose `security` field IS their ACL; replaces the previously-planned sim-side KB restriction fields) is indistinguishable from push on stage and needs no inbound connectivity.
...
- **`bench/conformance_bench.py` (Tier A1, ~3 ph + 1.5 ph oracle):** generates the sponsor's *exact protocol topology* (5 levels, 20 roles, 1,000 ACL-tagged docs) plus **10,000 ground-truth allow/deny queries**, and emits `bench/RESULTS.md` as a table of *measured value vs published threshold vs pass/fail* for every metric above, raw JSON committed. **Independent oracle (round-2 judge catch — non-negotiable):** expected labels must NOT come from the effective-policy compiler being measured (FNR=0 by construction = the exam grading itself). A separate naive reference implementation — a direct lineage-conjunction walk over `acl_source` with no bitmaps, ~40 lines — produces the expected labels; report FNR/FPR as bitmap-vs-oracle disagreement, and quote the oracle in the README. Statistics note (builder-verified): FNR <0.1% is only claimable with **≥3,000 deny-expected queries at zero leaks** (rule of three) — the 10k protocol provides this. **Decoupling note (BasedAI judge): the synthetic-topology bench + oracle need ZERO product code** — hand them to the bounded-technical contributor starting Friday morning, in parallel with the product build; run the synthetic bench + ≥4/6 attacks against the frozen build **before the 21:00 freeze** and commit those numbers into the PR Friday night (Saturday's push then only *adds* the UCI realism run — the PR is never numbers-free under the stricter deadline reading). This also removes the conformance bench from the "BasedAI extras" cut-line: only the UCI realism run and Tier C items remain cuttable.
...
- **`tests/test_audit_coverage.py` (Tier A1, ~1 ph):** asserts every retrieval/denial/sync/execution path writes an audit row (trivial — `permitted()` already calls `audit()`); README "Audit posture" paragraph: hash-chain verification command, 100%-coverage test reference, retention design note (the doc cites 7-year FINTRAC as its example), **plus the corroboration line: every ACL change Precedent acts on also exists as a timestamped record in Jira's own audit log (auditing API verified live) — two independent logs that cross-reference.**
- **Live drift/TTC measurement (Tier A2, ~1 ph net):** scripted N-flip run against the real Jira APIs (role membership on 10007/10008 AND issue-security levels on runbook issues): wall-clock from flip → recompiled-bitmap deny (TTC vs <5 min bar; expect seconds at 2–3 s polling) and stale-allow fraction during the window (drift vs <0.5%). **External clock (Standard, verified): each flip's timestamp is taken from Jira's own audit records API (`/rest/api/3/auditing/record`, ms precision) rather than our client clock — the TTC numbers are anchored to the source system's regulator-grade log.** A claim BioVault's synthetic standalone structurally cannot make.
...
### 2.8 The revocation demo moment — now DUAL-ENFORCEMENT (upgraded 3 Jul after the Standard verification; 60 seconds, scripted for video + Q&A party trick)

1. Split screen: the runbook issue in Jira (a real MEDIA issue, currently visible) + Precedent memory browser showing the derived executed-fix record retrievable as the `scheduling-ops` principal.
2. Presenter sets the issue's security level to **"Rights Ops Only"** in Jira (one API call / one UI dropdown — this exact mechanic was live-tested 3 Jul).
3. Two independent enforcement layers react to the same flip: **Jira itself starts returning 404** for the scheduling principal (measured propagation ≤5 s), and within one Precedent poll tick (≤3 s) the console flashes `acl_sync_applied` → the same memory query is **denied**, embedding path included (show the vector row still existing but unreachable).
4. Punchline: *"One permission change. The source system hides the runbook, and everything Precedent ever derived from it — summaries, fix records, embeddings — goes dark in seconds. Two enforcement layers, two audit logs: ours is hash-chained, and Jira's own audit record of the flip is right here, timestamp-matched."* (`GET /rest/api/3/auditing/record` — verified live, 593 records.)
5. Flip it back; both layers restore; second audit event pair. (Restoration proves it's live, not a canned clip.)

Stated consistency window (README + Q&A, measured): Jira-side visibility propagation ≤5 s + Precedent poll ≤3 s ⇒ **flip→dark ≤8 s worst case, atomically once written locally**.
...
- ~~Provision the JSM trial before noon today~~ **DONE — the site is on JSM STANDARD (upgraded + verified 3 Jul).** Full ACL-source API surface verified: roles 10007/10008 exist; role membership add/read/remove on MEDIA; grant flip on the live scheme 10000; **issue security created and enforcement-proven** (scheme "Precedent Restricted Runbooks"=10000, levels 10000/10001 mapped to the two ops roles, associated with MEDIA; Jira 404s a restricted issue to a non-member within ≤5 s of a role flip — timed test, fully restored after). The old Free-tier enforcement caveat is retired: **Jira-side enforcement is now real and demoable**, on top of Precedent's own retrieval-layer enforcement. Remaining 15-min human task: invite a 2nd agent seat (covered by the Standard trial) so the two ops principals are distinct humans on screen.
...
**Friday AM (unblockers — start all in parallel):**
0. **Hello-world Chat-Protocol Watcher registers on Agentverse in the first 30 minutes** (echo-level, both badges) — it starts the activity/≥10-interaction discoverability clock ~12 hours earlier than waiting for the real agent; the real Watcher later replaces its handler behind the same address. Then: BasedAI skeleton PR (see above). — **1 h combined**
1. `models.py` pin + Ollama fallback profile + the huggingface.co-`modelSource` startup assertion — **0.5 h** (Venice verification + evidence dumps DONE in the 3 Jul session)
2. ~~JSM trial provisioning~~ **DONE — Standard live and verified** (roles 10007/10008; live-scheme grant flips; issue-security scheme 10000 associated + enforcement-proven ≤5 s; auditing API live). Remaining: 2nd-user invite — **0.5 h human task, not on critical path**. Build note: seed the two restricted runbook issues (KB #4/#5 mirrors) with level "Rights Ops Only" during data seeding — the level IDs are in `.env`
3. Agentverse registration × 3 (mailbox agents, Chat Protocol, both badges, fresh-account ASI:One discovery test; **run 10+ real test chats through the day**; **capture an insurance shared-chat URL at echo/triage level by midday**, before the Jira write path exists) — **3 h remaining** (fetch judge: before any console polish)
4. `precedent_memory` schema + write/retrieve paths (A/B semantics) — **3 h**
4b. **(bounded-technical contributor, parallel all day):** `bench/conformance_bench.py` + independent oracle + adversarial suite — zero product-code dependencies (§2.7); synthetic results committed before the freeze — **~7 h**
...
10b. Extractor false-match measurement: run the 100-mutation corpus through the deterministic extractor **at 18:30, immediately after the vertical-slice gate** (the robustness chip's numbers must exist before the 20:00 run-through and the ~21:30 video recording — "overnight" is too late); report correct-match / safe-degrade / false-match rates → repo, slide 10, README first screen, BUIDL page — **2 h**
10c. **Checkpoint side-task:** if cut-line #3 fires (Librarian collapses in-process), sweep "three agents" from deck A7, video captions, and README before the freeze (the rehearsed two-agent E1 answer already exists) — **0.5 h contingent**
...
**FRIDAY MORNING (at stand-up — retimed after judge round 2):** BasedAI **skeleton draft PR opens FIRST THING**, not at 20:00 (the 20:00 slot collides with the second run-through and the pre-freeze push — exactly when unowned tasks die). Owner named at stand-up; fork, copy `UK-AI-Agent-EP5/submissions/_TEMPLATE/` verbatim (lowercase-hyphenated folder), README skeleton with the models declaration + `docs/compliance/` citations + all six adversarial test names + placeholder video link, own Venice-only `.env.example`, **no secrets**, only touch the team folder — **0.5 h now, content commits land through the day**. ⚠️ The live event README says the submission deadline is **"3 Jul (end of day)"** (marked TODO-confirm) while the track-brief Doc says 4 Jul before judging; CONTRIBUTING warns "late submissions may not receive evaluation". A morning draft PR satisfies every reading; synthetic-bench + attack results commit before the 21:00 freeze; the video link + UCI realism run push Saturday with a PR comment. Ask a BasedAI mentor which deadline governs and note the answer in the PR description. Template headings to follow verbatim (fetched live): `# <Project Name>` / `## What it does` / `## Demo` / `## How to run it` / `## How it works` / `## Tech & sponsor APIs used` / `## What's next` / `## License`; add the open-weight model declaration prominently (the template's generic .env.example lists ANTHROPIC/OPENAI keys — override with our Venice-only `.env.example`).
