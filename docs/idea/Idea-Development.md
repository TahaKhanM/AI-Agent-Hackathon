...
**NEW — our own measured numbers (computed 3 Jul on the real 141k-event UCI corpus, script committed in `data/analysis/`):** in 24,918 real incidents, **94.4% arrived with their exact fix class already precedented** (98.6% at symptom level) — **and those precedented repeats still took a median of 18.2 calendar hours to resolve by hand** (36% breached SLA; 47% were reassigned at least once). 558 recurring classes cover 94.8% of all volume — the autonomy ladder's bootstrap is measured, not asserted. **Arrival-time precision also measured** (round-2 hardest question, closed): a naive symptom-only baseline ranks the true fix top-1 59.4% / top-3 87.7% — the right fix is on the operator's first screen ~9 times in 10 before any AI, and the 59% top-1 is exactly why execution requires fingerprint equality, never rank-1 similarity. *(The previously-planned `knowledge=true` "killer stat" is inverted in the real data — confounded — and is demoted to labelled colour; see `data/analysis/uci-baseline-results.md`.)*
...
### Jira (upgraded to STANDARD and verified — the ACL story got stronger twice in one day)
The site now runs JSM **Standard** (license verified `PAID`). Everything the Free tier already proved still holds (roles `precedent-rights-ops`=10007 / `precedent-scheduling-ops`=10008; membership flips; grant flips on the live scheme), and Standard adds two verified capabilities the demo now uses:
1. **Issue security, enforcement-proven.** Scheme "Precedent Restricted Runbooks" (levels mapped to the two ops roles) is created and associated with MEDIA; a timed live test showed Jira itself returns **404 within ≤5 seconds** of a role flip on a restricted issue, and restores just as fast. Restricted KB runbooks (articles #4/#5) are mirrored as MEDIA runbook issues whose `security` field IS their ACL — the demo script's line "the presenter flips the article's restriction in Jira" is now literally true, and the RESTRICT moment becomes **dual-enforcement**: one flip → Jira hides the runbook AND Precedent's derived memory goes dark within a poll tick (flip→dark ≤8 s worst case, measured components).
2. **Jira's audit records API is live** (593 records, ms timestamps) — the drift/TTC bench anchors its flip timestamps to the source system's own regulator-grade log, and the README's audit posture cites two independent, cross-referencing logs.
Remaining 15-min human task: invite a 2nd agent seat (covered by the trial) so the two ops principals are distinct humans; single-account role-flip is the fallback. Write-behind cache + replay for stage safety; **Window B cuts only on a green sync-tick**.
...
2. ~~Verify Venice model IDs~~ ✅ done (evidence committed). ~~JSM Standard trial~~ ✅ **provisioned and verified** — issue security + audit API live; scheme/level IDs in `.env`.
3. Invite the 2nd Jira agent seat (15 min, covered by the trial) and record its accountId in `.env`; add it to role 10007 so the incident-3 deny/allow split is two named humans.
4. ~~Run the UCI match-rate computation~~ ✅ done this session — 94.4% / 18.2h / 558-class table in `data/analysis/`.
5. Name the BasedAI PR owner **at Friday stand-up** and open the **skeleton draft PR immediately** (template-verbatim, models declaration, six named attack tests, `docs/compliance/` citations); commit synthetic bench + attack results before the 21:00 freeze; push the video link Saturday with a PR comment; ask a BasedAI mentor which deadline governs and note the answer in the PR description.
...
7. Ratify in writing at stand-up: the demo-mode gate rule (04 §4.3), the 13:00/17:00 checkpoints, the 22:00 selection-announcement branch (selected → attract-mode + RESTRICT hotkey; not selected → 90s cut + video inserts), and — round-2 Technical fatal — **a named-person hour grid for Friday/Saturday** (the specs deliberately assign workstreams, not people; the grid is the team's, and the conformance bench belongs to the bounded-technical contributor from Friday morning).
8. Courtesy-confirm with Gautam (Fetch Discord) that DoraHacks is the only submission channel.

## 7. Decision record

Locked 3 Jul (user): Precedent ▪ media-vertical demo ▪ live Jira SM ▪ Fetch committed ▪ BasedAI-aligned ▪ $600B headline ▪ Disney+ named ▪ L3 = "Standing Approval". Session-adopted (v5, within unlocked space — full tiered list and hour ledger in [refinement/06-session-working-notes.md](refinement/06-session-working-notes.md) §7): sponsor-vocabulary conformance bench ▪ 6/6 adversarial suite ▪ extractor accuracy measurement + robustness chip ▪ BasedAI PR moved to Friday night ▪ Devpost refuted ▪ JSM-Free finding ▪ UCI numbers on-slide ▪ no-stage submission-surface package ▪ checkpoint rule ▪ hosted Watcher Friday night ▪ agent-hop trail + TVmaze snapshot quote in chat ▪ temporal-embargo demoted to bench-test-only (no demo beat). Rejected (one line each, ibid.): on-chain Payment Protocol; dual-surface ticket; mid-demo degraded toggle; judge-ticket-as-incident-2; final-slide counter wiring. Round-2 amendments: independent FNR/FPR oracle added; conformance bench decoupled from product code (bounded contributor, results before freeze); skeleton PR at stand-up; hosted Watcher promoted to A1; arrival-time precision measured in-session. Ledger closing balance (corrected after the round-2 audit): **≈73.1 ph committed / 77.1 with schedule-gated Tier A2 / ≈80.1–80.6 absolute worst case with a Tier-B branch** against 60–80 capacity — with a pre-ratified auto-drop rule (close strip + one A2 item) if the worst case materialises; Tier C unfunded below the 80-end.
